...
async function main() {
  ...
  const userRecords: { [key in UserRole]?: { id: string; email: string; role: UserRole } } = {};
  ...
  const customer = await prisma.customer.upsert({
    ...
    update: {},
    create: {
      userId: userRecords[UserRole.CUSTOMER]?.id ?? (() => { throw new Error("Customer user record is undefined."); })(),
  ...
  const project = await prisma.project.create({
    data: {
      ...
      createdById: userRecords[UserRole.CUSTOMER]?.id ?? (() => { throw new Error("Customer user record is undefined."); })(),
    },
  });
