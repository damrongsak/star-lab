...
export class SettingsController {
  ...
  async updateSettings(req: Request, res: Response): Promise<void> {
    try {
      ...
      const settingsArray = Object.entries(settingsToUpdate).map(([key, value]) => ({
        key,
        value: String(value),
  ...
  private inferCategory(key: string): string {
    ...
    if (key.startsWith("lab") || key.includes("Turnaround") || key.includes("Samples") || key.includes("Doctor")) return "LAB";
    return "GENERAL";
  }
}
