...
export class CustomerProjectController {
  ...
  async getProjects(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await customerProjectService.getProjectsByCustomer(
        customer.id,
        {
          page,
          limit,
      ...
      const projectsWithStats = await Promise.all(
        result.data.map(async (project) => {
          const stats = await customerProjectService.getProjectStats(
            project.id,
          );
  ...
  async updateProject(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const existingProject =
        await customerProjectService.getProjectById(projectId);
