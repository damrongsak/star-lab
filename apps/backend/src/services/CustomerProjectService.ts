...
export class CustomerProjectService {
  ...
  async createProject(data: CreateProjectData): Promise<Project> {
    try {
      ...
      if (existingProject) {
        throw new Error(
          `Project code '${data.projectCode}' already exists for this customer`,
        );
      }
      ...
      logger.info(
        `Created project ${project.projectCode} for customer ${data.customerId}`,
      );
      return project;
    } catch (error) {
  ...
  async getProjectStats(projectId: string) {
    try {
      ...
      const totalAmount = invoices.reduce(
        (sum, invoice) => sum + toNumber(invoice.netTotal),
        0,
      );
